...
describe("Added unit tests",function () {
  it("check dead items are removed in update",function (done) {
    const game = {
      ship:createSpaceship(),
      asteroids:[createRandomAsteroid().dead = true ,createRandomAsteroid(),createRandomAsteroid()],
      lasers:[],
      score:1,
      lives:3,
      level:1
    ...
    try {
      ...
      expect(updated.asteroids.length).to.equal(2)
    }
    catch(error) {
      done(error);
      return;
